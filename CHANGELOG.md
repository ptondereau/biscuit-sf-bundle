...
- `make:biscuit-policy` is now registered with MakerBundle when that bundle is installed. The maker class shipped since the beginning but was never wired as a service, so the command did not exist in any application.
...
- `BiscuitTokenManager` and `RevocationChecker` are no longer `final`, replacing the interfaces that existed only to make them mockable and decoratable (see Removed).
- `Token\Template\Applier::populate()` accepts a `BiscuitBuilder`, `BlockBuilder` or `AuthorizerBuilder` directly. The three builders expose identical `addFact`/`addRule`/`addCheck` methods, so the adapter layer added nothing.
- A `--param` value without `=` is now skipped by every command. `biscuit:token:create` used to throw where `biscuit:token:attenuate` and `biscuit:policy:test` skipped; a parameter a template needs but does not receive still fails as an unbound Datalog parameter.
...
- `biscuit.keys.algorithm` and `KeyManager::getAlgorithm()`. Nothing ever read them: hex keys carry their algorithm in the `ed25519/<hex>` prefix, PEM is self-describing, and `biscuit:keys:generate` has its own `--algorithm` option. Delete the key from your configuration.
- `Token\BiscuitTokenManagerInterface` and `Revocation\RevocationCheckerInterface`, each of which had exactly one implementation. Type against `Token\BiscuitTokenManager` and `Revocation\RevocationChecker`; both classes are open for mocking and decoration.
- `Token\Template\TemplatableBuilder` and the three builder adapters (`BiscuitBuilderAdapter`, `BlockBuilderAdapter`, `AuthorizerBuilderAdapter`). Pass the native builder straight to `Applier::populate()`.
- The compiler check that a manually tagged revocation service implements the matching store, writer or enumerable interface. Autoconfiguration derives the tags from the interfaces, so the check could only re-verify what the type system already guaranteed; a hand-tagged wrong class now fails at runtime instead of at compile time.
